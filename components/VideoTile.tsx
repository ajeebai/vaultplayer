...
export const VideoTile: React.FC<VideoTileProps> = ({ video, onSelectVideo, onToggleFavorite, onUpdateTags, onUnsupportedMedia, onPrioritizeMedia, isGrid = false }) => {
  ...
  const tileClassName = `flex-shrink-0 ${isGrid ? 'w-full' : 'w-64'} aspect-video bg-brand-gray rounded-lg overflow-hidden cursor-pointer group relative shadow-lg transition-all duration-300 hover:shadow-xl hover:shadow-brand-red-glow`;
