...
const HeartSolidIcon: React.FC<{className?: string}> = ({className}) => (<svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" /></svg>);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button onClick={(e) => handleActionClick(e, () => onToggleHidden(video.fullPath))} title={video.isHidden ? "Unhide" : "Hide"} className="bg-black/60 rounded-full p-2 text-white hover:bg-white hover:text-black transition-colors"><EyeSlashIcon className="w-4 h-4" /></button>
        <button onClick={(e) => handleActionClick(e, () => setIsEditingTags(true))} title="Edit Tags" className="bg-black/60 rounded-full p-2 text-white hover:bg-white hover:text-black transition-colors"><TagIcon className="w-4 h-4" /></button>
        <button onClick={(e) => handleActionClick(e, () => onToggleFavorite(video.fullPath))} title="Favorite" className="bg-black/60 rounded-full p-2 text-white hover:bg-white hover:text-brand-red transition-colors">
          {video.isFavorite ? <HeartSolidIcon className="w-4 h-4 text-brand-red" /> : <HeartIcon className="w-4 h-4" />}
        </button>
      </div>
